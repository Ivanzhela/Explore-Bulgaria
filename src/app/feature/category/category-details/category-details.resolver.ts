...
export class CatgoryDetailsResolver implements Resolve<PlacesCollection> {
  ...
  resolve(
    ...
    | Observable<PlacesCollection>
    | Promise<PlacesCollection>
    | PlacesCollection {
    ...
    const defaultCategoryQuery =
      enumMapping[category][Object.keys(enumMapping[category])[0]];
    const subcategoryQuery = enumMapping[category][subcategory];
    const query =
      subcategory !== undefined ? subcategoryQuery : defaultCategoryQuery;
